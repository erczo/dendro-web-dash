<template>
  <div class="component d-flex flex-row justify-content-center">
    <div class="d-flex flex-column o-hidden" :class="media.length > 1 ? 'rounded-left' : 'rounded'" v-if="media.length > 0">
    ...
    <div class="d-flex flex-column bg-faded o-hidden rounded-right" style="border-left: solid 1px #fff;" v-if="media.length > 1">
      <a role="button" style="position: relative;" v-for="(item, index) in media" v-if="index > 0 && index < 5" @click.prevent="select(index)">
        ...
        <div class="d-flex flex-column justify-content-center w-100 h-100" style="top: 0; left: 0; position: absolute;" v-if="index > 3 && media.length > 5">
